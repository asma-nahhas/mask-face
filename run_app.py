...
import numpy as np #for the math equitions
import tensorflow as tf #to open video cam
import os
from collections import defaultdict
from sqlalchemy.orm import sessionmaker
from tabledef import *
from keras import backend as K
engine = create_engine('sqlite:///login_db.db', echo=True)#create SQlite table for login credentials

import pickle
import cv2
import os.path
#from utility import img_to_encoding, resize_img
#to resize and encode the face image
...
def face_present(image_path):
    ...
    for (x, y, w, h) in faces:
        ...
        cv2.putText(img,"Asma",(x,y),cv2.FONT_HERSHEY_DUPLEX,2,(255,0,0),4)
    return face_present
...
def face_recognition(encoding, database, model, threshold=0.6):
    ...
    authenticate = False
    print('in face recognition function')
...
@app.route('/dashboard')
def dashboard():
    print('Im in dashboard function')
